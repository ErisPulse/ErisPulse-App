...
- 实例详情新增「拓扑」视图：聚合 `/api/topology`，以归属树展示模块的命令 /
  服务 / 事件处理器 / 路由 / 生命周期钩子 / 依赖，以及适配器 → Bot 归属与
  作用域绑定；SDK 过旧时显示优雅降级提示
- 监控视图新增「性能」子页：CPU / 内存实时折线图（fl_chart，3 秒采样，
  保留最近约 4 分钟窗口）
- 桌面快捷键：F5 / Ctrl+R 刷新，Ctrl+1/2/3 切换 实例 / 设置 / 调试，
  Ctrl+W 走关闭确认
- 桌面无边框窗口四角圆角（MaterialApp 层全局裁剪，所有路由页统一生效）

### 修复

- Windows 进程日志中断（"日志不全"）：子进程 stdio 强制 UTF-8
  （`PYTHONUTF8` / `PYTHONIOENCODING`），解码容忍坏字节并挂 `onError`，
  解码异常不再杀死日志订阅；Android 侧同样加固
- 删除实例后同名重建"变成之前的实例"：工作目录改由实例短 id 派生（与
  名字解耦）；端口分配探测系统真实占用并跳过被监听端口；显式指定被占用
  端口时直接报错；删除流程等待进程退出并清理 venv 后再删元数据
- `_update` 更新 token 时与替换后的新值比较导致永不落盘：改为与旧值比较，
  并新增 `updateToken` 公开方法（含回归测试）
- `load()` 单条损坏记录导致全部实例"消失"：改为逐条解析跳过损坏条目，
  原始数据先备份（`erispulse.instances.v2.corrupt-backup`）防止覆盖丢失
- 文件浏览 / 读取路径未 URL 编码：文件名含空格、`#`、`%` 时不再错乱
- 环境等待 Timer 提前完成后不再泄漏

### 优化

- 对齐 Dashboard：动态取色失败时回退品牌蓝（亮 `#4AA3DD` /
  暗 `#6A9DF0`），不再回落 Material 默认紫
- 状态语义色收敛到 `instanceStateColor` 单点维护（对齐 Dashboard 芯片
  配色，明暗自适应）
- 补齐遗漏的多语言文案（通知渠道 / 托盘菜单 / 错误视图 / 创建页提示 /
  模型层标签），托盘与通知渠道文案跟随应用语言
...
- 桌面无边框窗口（`window_manager` / `tray_manager`）：全部页面共用同一
  自绘顶栏，空白区拖动窗口、双击最大化 / 还原、最小化 / 最大化 / 关闭按钮
  齐全；系统托盘左键恢复窗口、右键菜单退出；关闭走确认（询问 / 最小化到
  托盘 / 停止并退出，可记住选择）
- 首页实例列表自适应布局：随窗口宽度在单列 / 两列 / 三列网格间切换
  （≥1150 三列 / ≥760 两列 / 否则单列），卡片撑满内容区
- 退出时「正在停止实例」过渡页面：停止全部实例期间显示提示，防止用户
  误以为卡死而强行关闭

### 变更

- 概览页重构为大屏运营 dashboard 风格：顶部状态横幅（名称 + 状态 / 健康
  徽章）、Hero 大数字 4 格（适配器 / 模块 / 在线机器人 / 事件总数，点击
  跳转对应视图）、系统资源大进度（CPU / 内存 + 运行时长 / PID / 线程 +
  框架版本）、机器人平台分布（按平台分组在线 / 总数）、消息统计、最近
  事件；宽屏左右两列（资源 flex 3 : 平台分布 flex 2），手机单列、大数字
  恒为 2×2（列数阈值与外层宽窄屏判断统一，避免窄屏溢出）
- 移动端首页实例列表由 `ListTile` 改为圆角卡片，状态点 / 名称 / 状态文本 /
  端口标签分层排布，统一 M3 卡片风格
- Android APK 改为纯 arm64-v8a 构建（179MB，原全平台胖包 196MB）：runtime
  二进制本就只有 aarch64；纯 64 位设备（骁龙 8 Gen 3 / 天玑 9300 等）的
  ROM 会拒绝安装含 32 位库的 APK。经 `abiFilters` + `disable-abi-filtering`
  过滤所有来源（引擎 / 插件 AAR / jniLibs），CI 产物不再提供 universal 胖包
- 桌面左侧导航与主区 `IndexedStack` 联动切换，移除主页内容区居中限宽

### 修复

- 实例未启动时点击其它视图（监控 / 事件 / 日志等）报连接错误：改为统一
  「实例未启动」引导占位（可一键启动），启动完成后自动连接并切换真实
  视图，无需手动刷新
- 「停止实例并退出」退出卡住：停止实例改为并行且逐层超时（子进程调用
  全部异步，不阻塞 UI），停止完成后直接终止进程（先隐藏窗口避免残留
  冻结帧），秒级确定退出；无本地运行实例时不弹过渡页直接快速退出
- 启动期概率性「页面加载但无法点击 / 主题色回退默认紫色」：窗口初始化
  未等待完成即 runApp，且 setAsFrameless 与 setTitleBarStyle 混用导致
  窗口样式竞争（FlutterView 子窗口几何与客户区错位、取色通道未就绪）。
  改为完整等待窗口就绪后再启动 UI，并统一只用一种无边框机制

## [0.2.4] - 2026/08/15

### 新增

- 新增全局主题 `lib/theme/app_theme.dart`：统一 Material 3 组件样式
  （卡片 / 按钮 / FAB / Chip / 输入框 / 对话框 / 底部弹层 / SnackBar 的
  圆角与柔和阴影，无渐变），跨平台视觉一致，颜色仍由 dynamic_color 与
  跟随系统决定
- 界面统一为「单条顶栏」布局：
  - 桌面端移除左侧导航栏（NavigationRail），logo + 标题 + 设置/调试/刷新
    入口整合进顶部栏，内容区居中自适应，去掉重复标题
  - 移动端 AppBar 保持同一风格，两端视觉一致
- 实例卡片：圆角卡片化，端口/版本改为圆润标签展示
- 空状态：图标改为柔和圆角底色容器，统一留白

### 变更

- 删除桌面端 NavigationRail 相关导航（`_onRailSelected` 等）
