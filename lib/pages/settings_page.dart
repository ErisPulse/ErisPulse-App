...
class SettingsPage extends StatefulWidget {
  ...
  /// 桌面主区域嵌入式显示（无独立 Scaffold/AppBar，顶部标题栏由主界面承载）。
  /// 移动端仍作为独立路由页（embedded=false，默认）。
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _confirmStopAll(RuntimeController runtime) async {
    ...
    if (ok == true) await runtime.stopAll();
  }
                      ...
                      children: [
                        for (final (code, label)
                          ...
                          ChoiceChip(
                            ...
                            onSelected: (_) => settings.setDownloadSource(code),
                          ),
